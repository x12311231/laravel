...
it('should create paychecks for salary employees', function () {
    $employees = Employee::factory()
        ...
        ->sequence(
            [
                'salary' => 50000 * 100,
                'payment_type' => Payment::SALARY->value
            ],
            [
                'salary' => 70000 * 100,
                'payment_type' => Payment::SALARY->value
...
it('should create paychecks for hourly rate employees', function () {
    ...
    travelTo(Carbon::parse('2022-02-10'), function () {
        ...
        $employee = Employee::factory([
            'hourly_rate' => 10 * 100,
            'payment_type' => Payment::HOURLY_RATE->value,
...
it('should not create paychecks for hourly rate employees without time logs', function () {
    travelTo(Carbon::parse('2022-02-10'), function () {
        $count = \App\Models\Paycheck::all()->count();
        $employee = Employee::factory([
            'hourly_rate' => 10 * 100,
            'payment_type' => Payment::HOURLY_RATE->value,
        ])->create();
        ...
        assertDatabaseCount('paychecks', $count);
    });
})->skip($skip);
...
it('should return all paychecks for an employee', function () {
    ...
    $testResponse = getJson(route('employee.paychecks.index', ['employee' => $employee, 'include' => 'employee']));
    $paychecks = $testResponse
        ->json('data');
